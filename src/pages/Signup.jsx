...
export default function Signup() {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true)
    try { signup(form); toast.success('Welcome to ScriptVault!'); navigate('/dashboard') }
    catch (err) { setError(err.message) }
    finally { setLoading(false) }
  }

  return (
    <AuthLayout title="Create your account" subtitle="Free, local-first, ready in seconds."
      footer={<>Have an account? <Link to="/login" className="font-medium text-amber-accent">Log in</Link></>}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <Input label="Name" required value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="Jane Doe"/>
      ...
      <Button variant="secondary" className="w-full" onClick={() => { loginAsGuest(); navigate('/dashboard') }}>Continue as guest</Button>
    </AuthLayout>
  )
}
